...
bool HTTPClient::begin(WiFiClient &client, String url) {
#ifdef HTTPCLIENT_1_1_COMPATIBLE
    if(_tcpDeprecated) {
        PR_INFO("mix up of new and deprecated api");
        _canReuse = false;
        end();
    }
#endif
    ...
    if(protocol != "http" && protocol != "https") {
        PR_ERR("unknown protocol '%s'", protocol.c_str());
        return false;
    }
...
#ifdef HTTPCLIENT_1_1_COMPATIBLE
    if(_tcpDeprecated) {
        PR_ERR("mix up of new and deprecated api");
        _canReuse = false;
        end();
    }
#endif
...
bool HTTPClient::begin(String url, const char* CAcert)
{
    ...
    if(!_transportTraits) {
        PR_ERR("could not create transport traits");
        return false;
    }
...
bool HTTPClient::beginInternal(String url, const char* expectedProtocol)
{
    ...
    if (_protocol != expectedProtocol) {
        PR_ERR("unexpected protocol: %s, expected %s", _protocol.c_str(), expectedProtocol);
        return false;
    }
    ...
    if(_host != the_host && connected()){
        PR_INFO("switching host from '%s' to '%s'. disconnecting first", _host.c_str(), the_host.c_str());
        _canReuse = false;
        disconnect(true);
    }
    _host = the_host;
    _uri = url;
    PR_INFO("protocol: %s, host: %s port: %d url: %s", _protocol.c_str(), _host.c_str(), _port, _uri.c_str());
    return true;
}
...
bool HTTPClient::begin(String host, uint16_t port, String uri)
{
    ...
    _uri = uri;
    _transportTraits = TransportTraitsPtr(new TransportTraits());
    PR_INFO("host: %s port: %d uri: %s", host.c_str(), port, uri.c_str());
    return true;
}
...
void HTTPClient::disconnect(bool preserveClient)
{
    if(connected()) {
        if(_client->available() > 0) {
            PR_INFO("still data in buffer (%d), clean up.\n", _client->available());
                _client->flush();
        }

        if(_reuse && _canReuse) {
            PR_INFO("tcp keep open for reuse");
        } else {
            PR_INFO("tcp stop");
            _client->stop();
            if(!preserveClient) {
...
int HTTPClient::sendRequest(const char * type, uint8_t * payload, size_t size)
{
    ...
    do {
        ...
        PR_INFO("request type: '%s' redirCount: %d\n", type, redirectCount);
        ...
        if(payload && size > 0) {
            ...
            while(sent_bytes < size){
                ...
                if (sent == 0){
                    PR_ERR("Failed to send chunk! Lets wait a bit");
                    delay(100);
                    sent = _client->write(&payload[sent_bytes], size - sent_bytes);
                    if (sent == 0){
                        PR_ERR("Failed to send chunk!");
                        break;
                    }
                }
                sent_bytes += sent;
            ...
            switch (code) {
                ...
                case HTTP_CODE_TEMPORARY_REDIRECT: {
                    if (
                        ...
                        redirectCount += 1;
                        PR_INFO("following redirect (the same method): '%s' redirCount: %d\n", _location.c_str(), redirectCount);
                        if (!setURL(_location)) {
                            PR_ERR("failed setting URL for redirection\n");
                            // no redirection
                            break;
                        }
                ...
                case HTTP_CODE_SEE_OTHER: {
                    redirectCount += 1;
                    PR_INFO("following redirect (dropped to GET/HEAD): '%s' redirCount: %d\n", _location.c_str(), redirectCount);
                    if (!setURL(_location)) {
                        PR_ERR("failed setting URL for redirection\n");
                        // no redirection
                        break;
...
int HTTPClient::sendRequest(const char * type, Stream * stream, size_t size)
{
    ...
    if(buff) {
        ...
        while(connected() && (stream->available() > -1) && (len > 0 || len == -1)) {
            ...
            if(sizeAvailable) {
                ...
                if(bytesWrite != bytesRead) {
                    PR_INFO("short write, asked for %d but got %d retry...", bytesRead, bytesWrite);

                    // check for write error
                    if(_client->getWriteError()) {
                        PR_ERR("stream write error %d", _client->getWriteError());
                    ...
                    if(bytesWrite != leftBytes) {
                        // failed again
                        PR_INFO("short write, asked for %d but got %d failed.", leftBytes, bytesWrite);
                        free(buff);
                        return returnError(HTTPC_ERROR_SEND_PAYLOAD_FAILED);
                    }
                }
        ...
        if(size && (int) size != bytesWritten) {
            PR_ERR("Stream payload bytesWritten %d and size %d mismatch!.", bytesWritten, size);
            PR_ERR("ERROR SEND PAYLOAD FAILED!");
            return returnError(HTTPC_ERROR_SEND_PAYLOAD_FAILED);
        } else {
            PR_INFO("Stream payload written: %d", bytesWritten);
        }

    } else {
        PR_ERR("too less ram! need %d", HTTP_TCP_BUFFER_SIZE);
...
WiFiClient& HTTPClient::getStream(void)
{
    ...
    PR_ERR("getStream: not connected");
...
WiFiClient* HTTPClient::getStreamPtr(void)
{
    ...
    PR_ERR("getStreamPtr: not connected");
    return nullptr;
}
...
String HTTPClient::getString(void)
{
    ...
    if(_size > 0 || _size == -1) {
        ...
        if(sstring.reserve((_size + 1))) {
            ...
            PR_ERR("not enough memory to reserve a string! need: %d", (_size + 1));
        }
    }
...
bool HTTPClient::connect(void)
{
    if(connected()) {
        if(_reuse) {
            PR_INFO("already connected, reusing connection");
        } else {
            PR_INFO("already connected, try reuse!");
        }
     ...
     if(_transportTraits && !_client) {
        ...
        if(!_tcpDeprecated) {
            PR_ERR("failed to create client");
            return false;
        }
...
#endif
    if (!_client) {
        PR_ERR("HTTPClient::begin was not called or returned error");
        return false;
    }
#ifdef HTTPCLIENT_1_1_COMPATIBLE
    if (_tcpDeprecated && !_transportTraits->verify(*_client, _host.c_str())) {
        PR_ERR("transport level verify failed");
        _client->stop();
        return false;
    }	
#endif
    if(!_client->connect(_host.c_str(), _port, _connectTimeout)) {
        PR_ERR("failed connect to %s:%u", _host.c_str(), _port);
        return false;
    }
    ...
    PR_INFO(" connected to %s:%u", _host.c_str(), _port);
...
int HTTPClient::handleHeaderResponse()
{
    ...
    while(connected()) {
        ...
        if(len > 0) {
            ...
            if(headerLine == "") {
                PR_INFO("code: %d", _returnCode);

                if(_size > 0) {
                    PR_INFO("size: %d", _size);
                }

                if(transferEncoding.length() > 0) {
                    PR_INFO("Transfer-Encoding: %s", transferEncoding.c_str());
                ...
                if(_returnCode) {
                    ...
                    PR_INFO("Remote host is not an HTTP Server!");
...
int HTTPClient::writeToStreamDataBlock(Stream * stream, int size)
{
    ...
    if(buff) {
            ...
            if(sizeAvailable) {
                ...
                if(bytesWrite != bytesRead) {
                    ...
                    if(stream->getWriteError()) {
                        PR_ERR("stream write error %d", stream->getWriteError());
                    ...
                    if(bytesWrite != leftBytes) {
                        // failed again
                        PR_ERR("short write asked for %d but got %d failed.", leftBytes, bytesWrite);
                        free(buff);
        ...
        PR_ERR("connection closed or file end (written: %d).", bytesWritten);

        if((size > 0) && (size != bytesWritten)) {
            PR_ERR("bytesWritten %d and size %d mismatch!.", bytesWritten, size);
            return HTTPC_ERROR_STREAM_WRITE;
        }
...
{
    if(error < 0) {
        PR_ERR("error(%d): %s", error, errorToString(error).c_str());
        if(connected()) {
            PR_INFO("tcp stop");
            _client->stop();
        }
...
bool HTTPClient::setURL(const String& url)
{
    ...
    if (!url.startsWith(_protocol + ':')) {
        PR_INFO("new URL not the same protocol, expected '%s', URL: '%s'\n", _protocol.c_str(), url.c_str());
        return false;
    }
