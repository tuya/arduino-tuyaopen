...
static int _handle_error(int err, const char * function, int line)
{
    ...
    PR_ERR("[%s():%d]: (%d) %s", function, line, err, error_buf);
#else
    PR_ERR("[%s():%d]: code %d", function, line, err);
#endif
    return err;
}
...
{
    PR_INFO("==start_ssl_client==\r\n");
    char buf[512];
    int ret, flags;
    int enable = 1;

    if (rootCABuff == NULL && pskIdent == NULL && psKey == NULL && !insecure && !useRootCABundle) {
        PR_INFO("==start_ssl_client:%d==\r\n",insecure);
        return -1;
    }

    PR_INFO("Starting socket");
    ssl_client->socket = -1;

    ssl_client->socket = lwip_socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (ssl_client->socket < 0) {
        PR_INFO("ERROR opening socket\r\n");
    ...
    if (res < 0) {
        PR_INFO("select on fd %d, errno: %d, \"%s\"\r\n", ssl_client->socket, errno, strerror(errno));
        lwip_close(ssl_client->socket);
        ssl_client->socket = -1;
        return -1;
    } else if (res == 0) {
        PR_INFO("select returned due to timeout %d ms for fd %d\r\n", timeout, ssl_client->socket);
        lwip_close(ssl_client->socket);
        ssl_client->socket = -1;
        return -1;
    } else {
        ...
        if (res < 0) {
            PR_INFO("getsockopt on fd %d, errno: %d, \"%s\"\r\n", ssl_client->socket, errno, strerror(errno));
            lwip_close(ssl_client->socket);
            ssl_client->socket = -1;
            return -1;
        }
...
#define ROE(x,msg) { if (((x)<0)) { PR_ERR("LWIP Socket config of " msg " failed."); return -1; }}
    ...
    PR_INFO("Setting up the SSL/TLS structure...\r\n");
    ...
    if (alpn_protos != NULL) {
        PR_INFO("Setting ALPN protocols");
    ...
    if (1 || insecure) {
        ...
        PR_ERR("WARNING: Skipping SSL Verification. INSECURE!");
    } else if (rootCABuff != NULL) {
        PR_INFO("Loading CA cert");
        ...
        PR_INFO("Attaching root CA cert bundle");
        ...
        PR_INFO("Setting up PSK");
        // convert PSK from hex to binary
        if ((strlen(psKey) & 1) != 0 || strlen(psKey) > 2*MBEDTLS_PSK_MAX_LEN) {
            PR_ERR("pre-shared key not valid hex or too long");
            return -1;
        }
        ...
        if (ret != 0) {
            PR_ERR("mbedtls_ssl_conf_psk returned %d", ret);
            return handle_error(ret);
        }
    } else {
        return -1;
    ...
    PR_INFO("Setting hostname for TLS session...\r\n");
    ...
    while ((ret = mbedtls_ssl_handshake(&ssl_client->ssl_ctx)) != 0) {
        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            PR_INFO("SSL/TLS handshake err...\r\n");
            return handle_error(ret);
        }
        if((millis()-handshake_start_time)>ssl_client->handshake_timeout)
        {
            PR_INFO("mbedtls_ssl_handshake timeout...\r\n");
            return -1;
        }
    ...
    if (cli_cert != NULL && cli_key != NULL) {
        PR_INFO("Protocol is %s Ciphersuite is %s", mbedtls_ssl_get_version(&ssl_client->ssl_ctx), mbedtls_ssl_get_ciphersuite(&ssl_client->ssl_ctx));
        if ((ret = mbedtls_ssl_get_record_expansion(&ssl_client->ssl_ctx)) >= 0) {
            PR_INFO("Record expansion is %d", ret);
        } else {
            PR_ERR("Record expansion is unknown (compression)");
        }
    }

    PR_INFO("Verifying peer X.509 certificate...\r\n");

    if ((flags = mbedtls_ssl_get_verify_result(&ssl_client->ssl_ctx)) != 0) {
        memset(buf, 0, sizeof(buf));
        mbedtls_x509_crt_verify_info(buf, sizeof(buf), "  ! ", flags);
        PR_INFO("Failed to verify peer certificate! verification info: %s\r\n", buf);
        return handle_error(ret);
    } else {
        PR_INFO("Certificate verified.\r\n");
    }
...
{
    PR_INFO("Cleaning SSL connection.");
...
int data_to_read(sslclient_context *ssl_client)
{
    ...
    //PR_ERR("RET: %i",ret);   //for low level debug
    res = mbedtls_ssl_get_bytes_avail(&ssl_client->ssl_ctx);
    //PR_ERR("RES: %i",res);    //for low level debug
...
{
    PR_INFO("Writing HTTP request with %d bytes...", len); //for low level debug
    int ret = -1;
    ...
    while ((ret = mbedtls_ssl_write(&ssl_client->ssl_ctx, data, len)) <= 0) {
        if((millis()-write_start_time)>ssl_client->socket_timeout) {
            PR_INFO("SSL write timed out.");
            return -1;
        }

        if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE && ret < 0) {
            PR_INFO("Handling error %d", ret); //for low level debug
            return handle_error(ret);
        }
...
{
    //PR_INFO( "Reading HTTP response...");   //for low level debug
    int ret = -1;

    ret = mbedtls_ssl_read(&ssl_client->ssl_ctx, data, length);

    //PR_INFO( "%d bytes read", ret);   //for low level debug
    return ret;
}
...
bool verify_ssl_fingerprint(sslclient_context *ssl_client, const char* fp, const char* domain_name)
{
    ...
    for (size_t i = 0; i < sizeof(fingerprint_local); ++i) {
        ...
        if (pos > len - 2) {
            PR_INFO("pos:%d len:%d fingerprint too short", pos, len);
            return false;
        }
        uint8_t high, low;
        if (!parseHexNibble(fp[pos], &high) || !parseHexNibble(fp[pos+1], &low)) {
            PR_INFO("pos:%d len:%d invalid hex sequence: %c%c", pos, len, fp[pos], fp[pos+1]);
            return false;
        }
        pos += 2;
    ...
    {
        PR_INFO("fingerprint doesn't match");
        return false;
    }
...
{
    if (!ssl_client) {
        PR_INFO("Invalid ssl_client pointer");
        return false;
    };

    const mbedtls_x509_crt* crt = mbedtls_ssl_get_peer_cert(&ssl_client->ssl_ctx);
    if (!crt) {
        PR_INFO("Failed to get peer cert.");
        return false;
    };
...
{
    PR_INFO("domain name: '%s'", (domain_name)?domain_name:"(null)");
    ...
    while (san != nullptr)
    {
        ...
        PR_INFO("SAN '%s': no match", san_str.c_str());
    ...
    while (common_name != nullptr)
    {
        ...
        if (!MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &common_name->oid))
        {
            ...
            PR_INFO("CN '%s': not match", common_name_str.c_str());
        }
